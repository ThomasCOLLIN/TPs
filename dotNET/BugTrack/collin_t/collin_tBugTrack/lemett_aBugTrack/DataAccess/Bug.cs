    ...
    public class Bug
    {
        ...
        public static T_Bug GetBug(long id)
        {
            try
            {
                ...
                model.T_Bug.Include("T_Transaction");
        ...
        public static DBO.BugReport GetBugAsBugReport(long id)
        {
            try
            {
                BugTrackEntities model = new BugTrackEntities();

                T_Bug bug = model.T_Bug
                    .Include("T_Project").Include("T_Transaction.T_User").Include("T_Comment")
                    .Where(x => x.id == id).FirstOrDefault();

                DBO.BugReport report = new DBO.BugReport();
                report.Title = bug.title;
                report.Type = "";
                report.Statut = bug.T_Transaction.LastOrDefault().statut.ToString();

                T_User responsable = bug.T_Transaction.LastOrDefault().T_User;

                report.Responsable = responsable.firstname + " " + responsable.name;

                bug.T_Comment.ToList().ForEach(com => report.Comments.Add(com.details));
        ...
        public static List<T_Bug> GetLastBugs(int max)
        {
